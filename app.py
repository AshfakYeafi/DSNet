...
# -----------------------------------------------------------------------------
# Auto-download weights if missing (Google Drive via gdown)
# -----------------------------------------------------------------------------
WEIGHT_DIR = "weight"
WEIGHT_FILE = os.path.join(WEIGHT_DIR, "weight.pth")
GOOGLE_DRIVE_FILE_ID = "17YzZiIGnXj7KGjxGxV89LEUQqeE3owZ8"  # <<< REPLACE THIS
...
def ensure_weights():
    """Download the weights into weight/ if they're missing."""
    if os.path.exists(WEIGHT_FILE) and os.path.getsize(WEIGHT_FILE) > 0:
        print(f"[INFO] Using weights at: {WEIGHT_FILE}")
        return
    os.makedirs(WEIGHT_DIR, exist_ok=True)
    ensure_gdown()
    import gdown
    print(f"[INFO] Downloading weights from Google Drive → {WEIGHT_FILE}")
    out = gdown.download(GOOGLE_DRIVE_URL, WEIGHT_FILE, quiet=False)
    if not out or not os.path.exists(WEIGHT_FILE) or os.path.getsize(WEIGHT_FILE) == 0:
        raise RuntimeError("Failed to download weights. Check GOOGLE_DRIVE_FILE_ID/link permissions.")

# Ensure weights exist before constructing/loading the model
ensure_weights()
...
MODEL = DSNet(in_channels=4, n_classes=3, n_channels=16).to(device=device)
...
def center_crop(data: np.ndarray, crop_height, crop_width, crop_depth):
    ...
    if height < crop_height or width < crop_width or depth < crop_depth:
        raise ValueError("Crop size larger than input volume.")
...
def get_survival_time(age, wt, tc, et):
    # Placeholder: your survival model can be integrated here
...
def process(images_name, _id, age):
    ...
    for file_path in images_name:
        ...
        img = (data - data_min) / (np.max(data) - data_min + 1e-8)  # add epsilon to avoid /0
        images.append(img)

    img = np.stack(images)
    # Original order kept: [4, 128, 128, 128] -> [B=1, C=4, D=128, H=128, W=128]
    img = np.moveaxis(img, (0, 1, 2, 3), (0, 3, 2, 1))
    result['Id'] = _id
    result['image'] = img

    imgs = torch.from_numpy(result['image']).unsqueeze(0)
    print(imgs.shape)

    with torch.no_grad():
        logits = MODEL(imgs.float().to(device))
        probs = torch.sigmoid(logits)
        predictions = (probs >= 0.33).float().cpu()
...
def upload_predict():
    if request.method == "POST":
        ...
        # Crop UI image (kept as in your original, but safer bounds)
        out_png = f'static/data/{image_name[:12]}_out.png'
        if os.path.exists(out_png):
            im = Image.open(out_png)
            width, height = im.size
            left, top = 25, 5
            right, bottom = max(left + 1, width - 175), height
            im1 = im.crop((left, top, right, bottom))
            im1.save(out_png)

        return render_template("index.html", image_loc=(f"{image_name[:12]}_out.png"),
                               prediction=image_name[:12], survival=survival)

    return render_template("index.html", prediction=0, image_loc=None)

if __name__ == "__main__":
    # Corrected: host is an address; port is numeric
